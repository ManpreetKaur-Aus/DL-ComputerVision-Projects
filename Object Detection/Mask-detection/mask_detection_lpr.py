...
def save_data(no_of_time_mask_detected, no_of_time_mask_crossed):
    ...
    try:   
        ...
        if q==today:
            ...
            w_sheet.write(sheet.nrows-1,2,w+no_of_time_mask_detected)
            w_sheet.write(sheet.nrows-1,3,e+no_of_time_mask_crossed)
            wb.save('result.xls')      
        else:
            w_sheet.write(sheet.nrows,0,sheet.nrows)
            w_sheet.write(sheet.nrows,1,today)
            w_sheet.write(sheet.nrows,2,no_of_time_mask_detected)
            w_sheet.write(sheet.nrows,3,no_of_time_mask_crossed)
            wb.save('result.xls')
        ...
        sheet.write(1, 2, no_of_time_mask_detected)
        sheet.write(1, 3, no_of_time_mask_crossed)
...
if __name__ == '__main__':
    ...
    Orientation=input("Enter the orientation of mask progression ~ lr,rl,bt,tb :")
    ...
    # max number of masks we want to detect/track
    num_masks_detect = 2
    ...
    try:
        ...
        # Read Frame and process
            ...
            a,b=detector_utils.draw_box_on_image(
                num_masks_detect, score_thresh, scores, boxes, classes, im_width, im_height, frame,Line_Position2,Orientation)
